...
export const CellAction: React.FC<CellActionProps>= ({
    ...
    const onCopy = (id: string) => {
        ...
        toast.success("productId copied to clipboard") 
    }

    const onDelete = async () => {
        try {
            setLoading(true)
            await axios.delete(`/api/${params.storeId}/products/${data.id}`) //kirim data ke folder api/stores/[storeId]/products/[productId] (logic api ini ada disitu)
            router.refresh()
            router.push(`/${params.storeId}/products`) // setelah dihapus, redirect ke halaman utama; push untuk navigasi ke halaman lain
            toast.success("Berhasil menghapus produk")
        } catch  {
            toast.error("Gagal menghapus produk, cek kembali data dan koneksi anda")
